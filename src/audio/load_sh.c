...
struct PendingDmaAudioBank* func_sh_802f4cb4(uintptr_t devAddr, void* vAddr, s32 size, s32 medium, s32 numChunks,
                                             OSMesgQueue* retQueue, s32 encodedInfo);
...
void* dma_sample_data(uintptr_t devAddr, u32 size, s32 arg2, u8* dmaIndexRef, s32 medium) {
    ...
    func_sh_802f3dd0(&gCurrAudioFrameDmaIoMesgBufs[gCurrAudioFrameDmaCount++], OS_MESG_PRI_NORMAL, OS_READ, dmaDevAddr,
                     dma->buffer, transfer, &gCurrAudioFrameDmaQueue, medium, shindouDebugPrint62);
...
void init_sample_dma_buffers(UNUSED s32 arg0) {
    ...
    sDmaBufSize = 0x2D0;
    sSampleDmas =
        sound_alloc_uninitialized(&gNotesAndBuffersPool, gMaxSimultaneousNotes * 4 * sizeof(struct SharedDma) *
    ...
    for (i = 0; i < gMaxSimultaneousNotes * 3 * gAudioBufferParameters.presetUnk4; i++) {
        if ((sSampleDmas[gSampleDmaNumListItems].buffer =
                 sound_alloc_uninitialized(&gNotesAndBuffersPool, sDmaBufSize)) == NULL) {
            break;
        }
...
void func_sh_802f50ec(struct PendingDmaAudioBank* arg0, size_t len) {
    ...
    func_sh_802f3dd0(&arg0->ioMesg, 0, 0, arg0->devAddr, arg0->vAddr, len, &arg0->dmaRetQueue, arg0->medium,
                     shindouDebugPrint110);
}
...
BAD_RETURN(s32) func_sh_802f5310(s32 bankId, struct AudioBank* mem, struct PatchStruct* patchInfo, s32 arg3) {
            ...
            switch (arg3) {
                case 0:
                    ...
                    if (temp != 0) {
                        if (temp_s0->size) {}
                        addr =
                            func_sh_802f1d90(temp_s0->size, patchInfo->bankId1, temp_s0->sampleAddr, temp_s0->medium);
                    } else {
                        temp = temp_s0->medium = patchInfo->medium2;
                        if (temp != 0) {
                            addr = func_sh_802f1d90(temp_s0->size, patchInfo->bankId2, temp_s0->sampleAddr,
                                                    temp_s0->medium);
                        }
                    }
                    break;
                ...
                case 1:
                    ...
                    if (temp != 0) {
                        addr =
                            func_sh_802f1d40(temp_s0->size, patchInfo->bankId1, temp_s0->sampleAddr, temp_s0->medium);
                    } else {
                        temp = temp_s0->medium = patchInfo->medium2;
                        if (temp != 0) {
                            addr = func_sh_802f1d40(temp_s0->size, patchInfo->bankId2, temp_s0->sampleAddr,
                                                    temp_s0->medium);
                        }
                    }
                    break;
    ...
    if (D_SH_8034F68C != 0 && sp4C == 0) {
        ...
        func_sh_802f4cb4(count, D_SH_8034EC88[D_SH_8034F68C - 1].ramAddr, temp_s0->size, temp_s0->medium, temp,
                         &gUnkQueue2, D_SH_8034EC88[D_SH_8034F68C - 1].encodedInfo);
    }
}
...
s32 func_sh_802f573c(s32 audioResetStatus) {
    ...
    next:
        if (D_SH_8034F68C > 0) {
            ...
            size = sample->size;
            func_sh_802f4cb4((uintptr_t)sampleAddr, D_SH_8034EC88[D_SH_8034F68C - 1].ramAddr, size, sample->medium, unk,
                             &gUnkQueue2, D_SH_8034EC88[D_SH_8034F68C - 1].encodedInfo);
        }
    }
    return 1;
