...
void falling_bowser_plat_act_check(void) {
    ...
    if (o->oSubAction == 0) {
        ...
        if ((gDebugInfo[DEBUG_PAGE_EFFECTINFO][6] + 20) * (o->oBehParams2ndByte - 1) < o->oBitsPlatformTimer) {
            o->oAction = BOWSER_BITS_PLAT_ACT_FALL;
        }
...
void falling_bowser_plat_act_fall(void) {
    ...
    if (!(o->oTimer & 1) && o->oTimer < 14) {
        angle = sBowserFallingPlatform[o->oBehParams2ndByte].angle + (gDebugInfo[DEBUG_PAGE_EFFECTINFO][1] << 8);
