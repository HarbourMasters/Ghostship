  ...
  if (ImGui::Begin("Save Editor", &mIsVisible)) {
    ...
    if (ImGui::BeginTabItem("Course Stars & Coins")) {
      for (int i = 0; i < COURSE_COUNT; i++) {
        ...
        UIWidgets::DrawFlagArray8(invisibleLabel, gSaveBuffer.files[gCurrSaveFileNum - 1][0].courseStars[COURSE_NUM_TO_INDEX(i)]);
        if (i < COURSE_STAGES_COUNT) {
          ...
          ImGui::InputScalar(invisibleLabel2, ImGuiDataType_U8, &gSaveBuffer.files[gCurrSaveFileNum - 1][0].courseCoinScores[COURSE_NUM_TO_INDEX(i)], NULL, NULL, "%u");
        }
      }
      ImGui::EndTabItem();
    }
