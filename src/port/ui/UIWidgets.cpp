...
void RenderText(ImVec2 pos, const char *text, const char *text_end, bool hide_text_after_hash) {
    ...
    if (text != text_display_end) {
        window->DrawList->AddText(g.Font, g.FontSize, pos, ImGui::GetColorU32(ImGuiCol_Text), text,
                                  text_display_end);
        if (g.LogEnabled)
...
bool Checkbox(const char *_label, bool *value, const CheckboxOptions &options) {
    ...
    if (options.alignment == ComponentAlignments::Right) {
        ...
        float labelOffsetX =
            above ? 0 : (lpFar ? labelFarOffset : (style.ItemInnerSpacing.x * 2.0f) + square_sz);
...
bool StateButton(const char *str_id, const char *label, ImVec2 size, ButtonOptions options,
    ...
    ImGui::RenderTextClipped(bb.Min + (style.FramePadding * 0.35f), bb.Max - (style.FramePadding / 4),
...
bool SliderInt(const char *label, int32_t *value, const IntSliderOptions &options) {
    ...
    if (options.showButtons) {
        if (Button("-", ButtonOptions{ .color = options.color }.Size(Sizes::Inline))
            && *value > options.min) {
            *value -= options.step;
            if (options.clamp) {
        ...
        ImGui::SetNextItemWidth(
            width - (ImGui::CalcTextSize("+").x + ImGui::GetStyle().FramePadding.x * 2 + 3) * 2);
    } else {
        ImGui::SetNextItemWidth(width);
    }
    if (ImGui::SliderScalar(invisibleLabel, ImGuiDataType_S32, value, &options.min, &options.max,
...
bool SliderFloat(const char *label, float *value, const FloatSliderOptions &options) {
    ...
    if (ImGui::SliderScalar(invisibleLabel, ImGuiDataType_Float, &valueToDisplay, &minToDisplay,
                            &maxToDisplay, options.format, options.flags)) {
...
bool InputString(const char *label, std::string *value, const InputOptions &options) {
    ...
    if (ImGui::InputText(label, (char *) value->c_str(), value->capacity() + 1, flags,
...
bool InputInt(const char *label, int32_t *value, const InputOptions &options) {
    ...
    ImGui::SetNextItemWidth(width);
    if (ImGui::InputScalar(label, ImGuiDataType_S32, value, nullptr, nullptr, nullptr,
                           options.addedFlags)) {
        dirty = true;
    }
...
bool CVarColorPicker(const char *label, const char *cvarName, Color_RGBA8 defaultColor, bool hasAlpha,
    ...
    if (hasAlpha) {
        changed =
            ImGui::ColorEdit4(label, (float *) &colorVec,
    ...
    if (showLock) {
        ...
        UIWidgets::CVarCheckbox(
            ...
            UIWidgets::CheckboxOptions({ { .tooltip = "Prevents this color from being changed" } })
                .Color(themeColor));
    }
    if (changed) {
...
bool RadioButton(const char *label, bool active, const RadioButtonsOptions &options) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddCircle(center + ImVec2(1, 1), radius,
                                    ImGui::GetColorU32(ImGuiCol_BorderShadow), num_segment,
