...
static RegisterShipInitFunc initFunc([]() {
    Regions[RR_LEVEL_BOB] = RandoRegion{ .regionName = "Bob Omb Battlefield", .levelId = LEVEL_BOB,
        .checks = {
            ...
            CHECK(RC_BOB_STAR_03_ISLAND,        HAS_COURSE_STAR(COURSE_BOB, RA_ACT_02) || CAN_USE(WING)),
            CHECK(RC_BOB_STAR_04_RED_COINS,     HAS_COURSE_STAR(COURSE_BOB, RA_ACT_02)),
            CHECK(RC_BOB_STAR_05_WINGS,         HAS_COURSE_STAR(COURSE_BOB, RA_ACT_02) && CAN_USE(WING)),
            CHECK(RC_BOB_STAR_06_CHAIN_CHOMP,   true),
            CHECK(RC_BOB_STAR_07_100_COIN,      HAS_COURSE_STAR(COURSE_BOB, RA_ACT_02)),
        },
    };
}, {});
// clang-format on