...
ARGV-element, is returned in `optarg_a'.  Two colons mean an option that
wants an optional arg; if there is text in the current ARGV-element,
it is returned in `optarg_a', otherwise `optarg_a' is set to zero.
...
%s: option `%c%s' doesn't allow an argument\n"),
...

 {
     ...
     if (temp[0] == 'W' && temp[1] == ';')
     {
         ...
         /* optarg_a is now the argument, see if it's in the
            table of longopts.  */

         for (nextchar = nameend = optarg_a; *nameend && *nameend != '='; nameend++)
             /* Do nothing.  */ ;
...
char **argv;
{
 ...
 while (1)
 {
     ...
     c = getopt_a (argc, argv, "abc:d:0123456789");
     if (c == -1)
         break;
